...
void SegAccuracyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->phase_ == TEST) {
    if (seg_acc_test_step_ > 
      this->layer_param_.seg_accuracy_param().test_iter()) {
      confusion_matrix_.clear();
      seg_acc_test_step_ = 1;
      LOG(INFO) << "Resting confusion matrix at step "<< seg_acc_test_step_;
    } else {
      seg_acc_test_step_ += 1;
    }
  }
